...
function SKS_RF_forward(Model,depth,dist,Gaussian_factor,Clean_Model::Int64=1,rotate::Int64=1)
...
SKS_rf = -[reverse(L_Srf[itzero+1:end]); reverse(L_Srf[1:itzero]) ]; # Rotated Receiver function for vertical component (positive upwards)
time = -reverse(ts_fft); #time samples, modified to get efficient fft.

return time, SKS_rf, Used_model
end
