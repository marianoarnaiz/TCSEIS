...
function S_RF_forward(Model,depth,dist,Gaussian_factor,Clean_Model::Int64=1,rotate::Int64=1)
...
p=Get_p(depth,dist,"S")
## Clean model 1 run the clean mode, 0 use the model as it is (May cause problems)
if Clean_Model ==1
    Model=Model[searchsortednearest(Model[:,1],0.5):searchsortednearest(Model[:,1],200),:];
...
S_shift = sum(sqrt.((1 ./VSs[2:N_layers+1].^2) .-p_sm^2) .*diff(zs',dims=1)); #apparent vertical traveltime of direct P-wave from bottom to surface.
Bu = [zeros(1,N_fft); shift(fs,-T_min_fft - S_shift)' ]; #upgoing wavefields below lowermost interface; impulsive P-wave at t=T_min_fft and no S-wave
...
up_Srf = reverse(real(ifft((H.*lump_UP_fd)./(lump_R_fd.*shift_fd)))); #note that the spectrum of UP normally has no zero crossings
r_Srf = real(ifft(H.*1.0.*shift_fd)); #%UP./UP = 1.0


## rotation angle
itzero =argmin(abs.(ts_fft));
if rotate == 0
    rot_ang = 0; #theoretical apparent incidence angle
    #println("No rotation")
else
    rot_ang = -atand(up_Srf[itzero]/r_Srf[itzero]) #theoretical apparent incidence angle
    #println("Considering Rotation, rot_ang = $rot_ang ")
end
##
...
L = real(ifft(H.*L_fd));
Q = real(ifft(H.*Q_fd));
Q_Srf = real(ifft(H.*1.0.*shift_fd));
L_Srf = reverse(real(ifft((H.*L_fd)./(Q_fd.*shift_fd))));
...
# Requested outpus
#R_rf=r_rf; #Unrotated Radial Receiver Function
#Z_rf=reverse(up_rf); #Unrotated Receiver function for vertical component (positive upwards)
#Q_rf = Q_Srf; #Rotated Radial Receiver Function
S2P_rf = -[reverse(L_Srf[itzero+1:end]); reverse(L_Srf[1:itzero]) ]; # Rotated Receiver function for vertical component (positive upwards)
time = -reverse(ts_fft); #time samples, modified to get efficient fft.

return time, S2P_rf, Used_model
end
...
- Removing the base of the Mantle (beyond 700 km)
...
Model=Model[searchsortednearest(Model[:,1],0.5):searchsortednearest(Model[:,1],250),:];
